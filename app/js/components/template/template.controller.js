angular.module("app.template", []).controller('TemplateController', function ($state, $stateParams, Templates, Beats, spinnerService, uuid2) {
  var self = this;
  self.id = $stateParams.id;
  self.template = {};

  if ($state.current.name === 'template-new') {
    // New template
    self.newTemplate = true;
    Templates.createBlankTemplate(self.id, $stateParams.mins).then(function(data) {
      self.template = data;
  ...
  if (!self.newTemplate) {
    ...
    Templates.loadTemplate(self.id, self.mins).then(function (data) {
      self.template = data;
      // TODO: remove this. Should only be in 'add'. Just here for testing.
      self.initFreestyleGoals();
    });
  }
